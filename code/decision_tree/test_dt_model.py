...
def predict(node_dict, feature_vector):
    ...
    # Get the Value for the feature from the Review's Feature Vector.
    feature_value = feature_vector.get(feature, 0)

    if feature_value <= threshold:
        return predict(node_dict["left"], feature_vector)
    else:
        return predict(node_dict["right"], feature_vector)

def main():
    if len(sys.argv) != 4:
        print("Usage: python test_dt_model.py <path_to_trained_dt_json> <path_to_testing_feature_json> <path_to_testing_data_csv>")
        sys.exit(1)
    ...
    for path in [dt_file_path, feature_file_path, test_file_path]:
    ...
    # Make Predictions.
    predictions = [predict(tree_json, vector) for vector in feature_test]
    ...
    if int(predictions[i]) == int(test_data['star_rating'].iloc[i])
    )
